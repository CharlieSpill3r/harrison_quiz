...
def main():
    ...
    if st.button("Submit Answers"):
        ...
        for q, comment in feedback.items():
            ...
            if response:
                st.write("Good attempt! Now, try rewriting it using this sentence starter: \"In the story, the government ensures that...\"")
    ...
    for question, answer in answers.items():
        if not answer.strip() or answer.lower().strip() in ["i don't know", "idk"]:
            feedback[question] = (f"Please re-read this section of the text: {reference_texts[question]}\n"
                                "Was everyone the same? (Your answer)\n"
                                "What examples in this section are given? (Your answer)\n"
                                f"How might you rewrite '{reference_texts[question].split('.')[1]}'? (Your answer)\n"
                                "Now, try rewriting it using this sentence starter: 'In the story, the government ensures that...'")
        else:
