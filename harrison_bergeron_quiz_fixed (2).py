...
def main():
    ...
    if st.button("Submit Answers"):
        ...
        for q, comment in feedback.items():
            ...
            if comment.startswith("Please re-read"):  # Engage students in step-by-step concept checking
                answer1 = st.text_input("Was everyone the same?")
                if answer1 and answer1.strip().lower() in ["no", "not really", "they were forced to be"]:
                    answer2 = st.text_input("What examples in this section are given?")
                    if answer2:
                        answer3 = st.text_input("How might you rewrite 'Nobody was better looking than anybody else'?")
                        if answer3:
